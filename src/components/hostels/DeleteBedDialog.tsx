...
import type { BedWithAllocationCount, BedActionResult } from "@/app/hostels/bed-actions";

interface DeleteBedDialogProps {
  isOpen: boolean;
  onClose: () => void;
  bed: BedWithAllocationCount | null;
  onConfirmDelete: (bedId: string) => Promise<BedActionResult>;
}
...
export function DeleteBedDialog({
  ...
  const isOccupied = bed.status === "occupied" || (bed.active_allocation_count ?? 0) > 0;

  const handleDelete = async () => {
    setError(null);
    setIsDeleting(true);
    try {
      const res = await onConfirmDelete(bed.id);
      if (!res.success) {
      ...
      <div className="relative w-full max-w-md rounded-2xl bg-slate-900 border border-slate-800 shadow-2xl overflow-hidden animate-in fade-in-0 zoom-in-95">
        ...
        <div className="flex items-center justify-between border-b border-slate-800 px-6 py-4">
          <div className="flex items-center gap-3">
            ...
            <h2 className="text-lg font-semibold text-white">Delete Bed</h2>
          </div>
          <button
            onClick={onClose}
            disabled={isDeleting}
          ...
          {isOccupied ? (
            <div className="rounded-xl bg-amber-500/10 border border-amber-500/20 p-4 space-y-2 text-amber-200 text-sm">
              <div className="flex items-center gap-2 font-medium">
                ...
                <span>Bed Currently Occupied or Allocated</span>
              </div>
              <p className="text-xs text-amber-200/80 leading-relaxed">
                Bed <strong>{bed.bed_label}</strong> is currently assigned or occupied.
                Deleting occupied beds or beds with active resident allocations is strictly prohibited to preserve housing data integrity.
              </p>
            </div>
          ) : (
            <div className="space-y-2">
              <p className="text-sm text-slate-300 leading-relaxed">
                Are you sure you want to delete <strong className="text-white">Bed {bed.bed_label}</strong>?
              </p>
              <p className="text-xs text-slate-400">
                This action will permanently remove the vacant bed record from this room configuration.
              </p>
            </div>
          )}
        </div>
